<template>
  <div>
    ...
    <el-empty v-else-if="reports.length === 0" description="还没有周期报告，点击右上角新建" />
...
const createReportType = ref('PERSONAL_WEEKLY')
...
const createTypeOptions = computed(() => grainCreateOptions('personal'))
...
async function loadReports() {
  ...
  try {
    ...
    const rt = listReportTypeForGrain('personal', periodFilter.value)
...
function openCreate() {
  if (createReportType.value === 'PERSONAL_WEEKLY') {
    createWeekly()
    return
  }
...
async function createWeekly() {
  try {
    ...
    router.push(`/app/period/${data.id}`)
  } catch (err) {
...
function onCreated(report) {
  ElMessage.success('报告已创建')
  router.push(`/app/period/${report.id}`)
}

function openReport(row) {
  router.push(`/app/period/${row.id}`)
}

function applyPeriodGrain(grain) {
  if (!grain) return
  periodFilter.value = grain
  createReportType.value = defaultCreateTypeForGrain('personal', grain)
}
